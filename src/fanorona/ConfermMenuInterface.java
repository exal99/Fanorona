	...
	protected void onResize() {
		if (lines != null) {
			...
			buttonFontSize = (int) Math.min(fitFontSizeToHeight(buttonHeight),
								 Math.min(fitFontSizeToWidth(buttonWidth, buttons.get(0).getCaptionLabel().getText()),
										  fitFontSizeToWidth(buttonWidth, buttons.get(1).getCaptionLabel().getText())));
			buttons.get(0).setFont(parrent.createFont("cour.ttf", buttonFontSize));
			buttons.get(1).setFont(parrent.createFont("cour.ttf", buttonFontSize));
		}
	}
